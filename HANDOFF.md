...
### 5.1 下一步待办（4任务+0.01 出结果后按序执行）

1. 同步新代码到云端（结果目录名任务数后缀，commit `dcc463a`；本地已改好，云端暂未同步以免打断训练中调用的推理脚本）
2. 云端 `/root/results/llama2-7b-chat/ratio_0.01` 改名 `ratio_0.01_4task` 归档
3. scp 拉回结果到本地 `results/llama2-7b-chat/ratio_0.01_4task/`
4. 回填 EXPERIMENT_LOG.md 第 10.6 节状态 + 结果表，上传关键结果到 GitHub
