...
- 🔄 **阶段 5：4任务+0.01 快速验证实验**（llama2 × 前4任务 × 0.01，独立于 15 组）✅ **已完成（2026-08-27）**：OP=0.5027 / BWT=+0.0480，对比 8任务 10% 基线（0.544/+0.077）OP -7.6%、BWT -0.029 → **下降阈值确认落在 1%~8% 之间**（方向性信号；正式对照需 8 任务补跑 0.01）。结果已归档改名 `ratio_0.01_4task`、拉回本地、回填 EXPERIMENT_LOG 10.6
- ⏸️ **阶段 4 主实验（剩余 12 组）**：llama2 0.05/0.02/0.01 + vicuna 5 组 + baichuan 5 组，等导师/学长验证 0.08 后继续

### 5.1 下一步待办（4任务+0.01 已完成，全部办结）

1. ✅ 同步新代码到云端（commit `dcc463a`；训练已结束，无打断风险，可在下次恢复实例时随代码一起同步）
2. ✅ 云端 `/root/results/llama2-7b-chat/ratio_0.01` 改名 `ratio_0.01_4task` 归档（2026-08-27）
3. ✅ scp 拉回结果到本地 `results/llama2-7b-chat/ratio_0.01_4task/`（.complete + manifest + 10 预测齐全）
4. ✅ 回填 EXPERIMENT_LOG.md 10.6 节（OP=0.5027/BWT=+0.0480）+ 上传关键结果到 GitHub

> 下一步主实验：阶段 4 剩余 12 组待导师/学长验证 0.08 后继续。注意：实例 8-27 曾定时关机，/dev/shm 已清空（模型/checkpoint/输出全丢），恢复需重新下载 llama2 模型 26G + 同步代码（含 dcc463a）。
...
7. **4任务 0.01 结果目录与未来 8任务 0.01 冲突（已解决）**：结果目录名固化任务数后缀——8 任务保持原名 `ratio_0.01`，非 8 任务用 `ratio_0.01_4task`（见 `run_replay_fast.sh`/`run_infer_parallel.sh` 的 `DIR_SUFFIX`）；`run_manifest.json` 的 `datasets`/`ntasks` 字段二次区分。
